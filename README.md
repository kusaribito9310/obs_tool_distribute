...
表示位置は右側のプレビューでドラッグできます。VC枠・チャット枠の辺または四隅をドラッグするとサイズを変更できます。Shiftキーを押しながら辺または四隅をドラッグすると、縦横比を保って内容全体を拡大・縮小できます。Discordの表示名をすぐ反映したい場合は「表示名を強制更新」を押してください。

1.0.17以降では、15秒ごとの表示名確認中もカテゴリ・VC・テキストチャンネルの一覧と選択を維持します。「本人の閲覧権限を確認中…」へ定期的に戻る場合や選択が消える場合は最新版へ更新してください。

画面の全項目、OBS設定、更新、権限の仕組み、症状別の対処は次のガイドにまとめています。

- [完全操作ガイド（Markdown）](USER_GUIDE.md)
- [完全操作ガイド（HTML）](USER_GUIDE.html)
