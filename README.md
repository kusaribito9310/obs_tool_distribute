...
1.0.18以降では、画像メイン表示の参加者名について背景色・透明度・文字サイズ（8～72px）を設定できます。「デザイン設定」の「画像の名前背景」「画像の名前サイズ」で調整し、プレビューで確認してから「設定を保存して反映」を押してください。VC枠・チャット枠・名前背景の透明度Aは、スライダーと数値欄のどちらでも0～1を0.01刻みで調整できます。表示済みチャットも、定期更新やローカル接続の一時切断では消えません。
...
1.0.19以降では、OBSシーンのスクリーンショットをツール右側のプレビュー背景に読み込み、映像を見ながらVC・チャットの位置とサイズを調整できます。背景画像は編集画面にだけ表示され、OBSのブラウザソースや設定ファイルには保存されません。詳しくは [操作ガイド](USER_GUIDE.md) を参照してください。
